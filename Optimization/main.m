...
if PRO
    gear_ratio = 6;
    motor_base_torque = 2.75; %Nm based on mini cheetah motor, saturation torque
    motor_base_free_speed = 210; %rads per second mini cheetah motor
    motor_torque_intercept = 38.5; %y-intercept of the power line Nm
end
...
if PRO
    ...
    p2 = [motor_base_free_speed/gear_ratio, 0];
    m_ts = (p2(2)-p1(2)) / (p2(1)-p1(1));
    b_ts = motor_torque_intercept;
end

%derive dynamics
[kinematics,dynamics] = derive_leg_MC(gear_ratio); 
if PRO
    [kinematics,dynamics] = derive_leg_MC_Pro(gear_ratio); 
end
...
t2 = dt:dt:2*full(t_peak); %simulate to peak
ys = [];
ts = [];
gs = [];
zeros = [];
